...
// ─── API CALLERS ──────────────────────────────────────────────────────────────
function checkArticle(d,target){const h=d?.html_content||"";if(!h)return "html_content absent";const words=h.replace(/<[^>]+>/g," ").split(/\s+/).filter(Boolean).length;const t=Number(target)||1500;if(words>t*2)return "trop long ("+words+" mots pour "+t+" demandes)";if(words<t*0.6)return "trop court ("+words+" mots pour "+t+" demandes)";const h2=[...h.matchAll(/<h2[^>]*>([\s\S]*?)<\/h2>/gi)].map(m=>m[1].replace(/<[^>]+>/g,"").trim().toLowerCase());const dup=h2.find((x,i)=>x&&h2.indexOf(x)!==i);if(dup)return "titre H2 duplique : "+dup;const ps=[...h.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi)].map(m=>m[1].replace(/<[^>]+>/g,"").trim()).filter(x=>x.length>80);const dp=ps.find((x,i)=>ps.indexOf(x)!==i);if(dp)return "paragraphe duplique : "+dp.slice(0,60);console.log("[Pipeline] Article controle OK:",words,"mots,",h2.length,"H2");return null;}
  ...
  async function runPipelineFor(subj,kw,wc,site,atype,lsc,autoPublish=false){
    ...
    try{
      for(const cfg of cfgs){
        ...
        try{
          const callStep=()=>callClaude(cfg.id==="article"&&wpCats.length?(()=>{const p=cfg.build();return{...p,user:p.user+"\n\nCATEGORIE WORDPRESS: choisis celle qui reflete le theme principal traite par l article (son sujet), pas l audience visee ni l etape du parcours du lecteur, et renvoie son nom exact dans le champ categorie : "+wpCats.map(x=>x.name).join(" | ")};})():cfg.build(),cfg.tokens,cfg.schema);let data;for(let attempt=1;;attempt++){let q=null;try{data=await callStep();if(cfg.id==="article")q=checkArticle(data,wc);}catch(err){q=err.message;}if(!q)break;console.warn("[Pipeline] Etape "+cfg.id+" rejetee (essai "+attempt+"):",q);if(cfg.id!=="article")throw new Error(q);if(attempt>=2)throw new Error("Article invalide apres 2 essais : "+q);}if(cfg.id==="article"&&!data?.html_content)throw new Error("html_content absent, cles recues: "+Object.keys(data||{}).join(","));
