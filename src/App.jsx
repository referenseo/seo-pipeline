...
[1] ${snippetInstr}
[2] INTRO (80 mots max): ${isLM?"hook vérité ligne 1 (jamais 'Dans cet article')":"accroche directe"}, contexte + mot-clé naturel.${scIntro?` Dernière ligne de l'intro: <!-- wp:shortcode -->${scIntro}<!-- /wp:shortcode -->`:""}
...
[5] CONCLUSION+CTA: ${isLM?"benefice concret AVANT l'action.":"CTA clair."}${scEnd?` Fin: <!-- wp:shortcode -->${scEnd}<!-- /wp:shortcode -->`:""}
      ...
      if(!abortRef.current&&acc.article&&!acc.article.error){
        if(profile.geminiKey){
          ...
          try{
            ...
            const is100jSite=freshSite?.name?.toLowerCase().includes("100jours")||freshSite?.wpUrl?.toLowerCase().includes("100jours")||freshSite?.name?.toLowerCase().includes("100 jours")||freshSite?.wpUrl?.toLowerCase().includes("100 jours");
